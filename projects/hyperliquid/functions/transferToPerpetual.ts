...
/**
 * Transfers funds from user's spot balance to perpetual (perp) balance on Hyperliquid by signing EIP-712 typed data.
 * @param amount - Amount of USDC to transfer
 * @param options - SDK function options
 * @returns Promise resolving to function execution result
 */
export async function transferToPerpetual({ amount }: Props, { signTypedDatas, notify }: FunctionOptions): Promise<FunctionReturn> {
    try {
        console.log('Starting spot->perp transfer with:', { amount });
